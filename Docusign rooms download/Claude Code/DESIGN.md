...
| 3 | Full per-room lifecycle status in the panel UI | **Partial.** The panel shows how many rooms are active and which ones, but not a full queued/in-progress/done/failed/empty breakdown per room - a minimal version landed alongside the worker-tab pool (step 5) since *some* multi-room visibility became necessary once more than one room could be in flight at once, but the richer version originally scoped here is still future work |
